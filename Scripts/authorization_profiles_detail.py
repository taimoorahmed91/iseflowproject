...
# Load authorization profiles from file
authorization_profiles_file = f"{OUTPUT_DIR}/authorization_profiles.json"
with open(authorization_profiles_file, 'r') as f:
    authorization_profiles_data = json.load(f)

# Extract hrefs from resources
hrefs = []
if 'SearchResult' in authorization_profiles_data and 'resources' in authorization_profiles_data['SearchResult']:
    for resource in authorization_profiles_data['SearchResult']['resources']:
        if 'link' in resource and 'href' in resource['link']:
            hrefs.append(resource['link']['href'])

print(f"Found {len(hrefs)} authorization profile(s) to fetch details for")
...
output_file = f"{OUTPUT_DIR}/authorization_profiles_detail.json"
with open(output_file, 'w') as f:
    json.dump(all_details, f, indent=2)

print(f"\n✓ Saved all authorization profile details to {output_file}")
print(f"  Total processed: {len(all_details)} authorization profile(s)")
