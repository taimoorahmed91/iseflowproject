...
# Store all authentication results
all_authentication_data = []

# Process each href
for idx, href in enumerate(hrefs, 1):
    # Parse the URL to extract the path
    parsed_url = urlparse(href)
    auth_path = f"{parsed_url.path}/authentication"

    print(f"\n[{idx}/{len(hrefs)}] Fetching authentication for: {auth_path}")
    ...
    try:
        ...
        result = {
            ...
            "authentication_url": f"https://{ISE_FQDN}{auth_path}",
            "data": json_data
        }
        all_authentication_data.append(result)

        print(f"  ✓ Successfully retrieved authentication data")

    except Exception as e:
        print(f"  ✗ Error: {str(e)}")
        all_authentication_data.append({
            "policy_set_href": href,
            "authentication_url": f"https://{ISE_FQDN}{auth_path}",
            "error": str(e)
        })
...
output_file = f"{OUTPUT_DIR}/authentication.json"
with open(output_file, 'w') as f:
    json.dump(all_authentication_data, f, indent=2)

print(f"\n✓ Saved all authentication data to {output_file}")
print(f"  Total processed: {len(all_authentication_data)} policy set(s)")
